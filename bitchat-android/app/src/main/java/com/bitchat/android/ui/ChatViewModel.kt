...
class ChatViewModel(
    ...
    // ASR (Nexa microphone recording)
    private val asrRecorder = com.bitchat.android.asr.AsrAudioRecorder(application.applicationContext)
    private val asrEngine = com.bitchat.android.asr.NexaAsrEngine(application.applicationContext)
    ...
    // MARK: - ASR (Nexa microphone recording)

    /**
     * Start ASR recording. Call from /asrstart or UI.
     */
    fun startAsrRecording() {
        viewModelScope.launch {
            if (!asrRecorder.hasPermission()) {
                messageManager.addMessage(BitchatMessage(
                    sender = "system",
                    content = "microphone permission required. grant it in app settings.",
                    timestamp = Date(),
                    isRelay = false
                ))
                return@launch
            }
            val file = asrRecorder.startRecording()
            if (file != null) {
                messageManager.addMessage(BitchatMessage(
                    sender = "system",
                    content = "recording started. use /asrstop to transcribe.",
                    timestamp = Date(),
                    isRelay = false
                ))
            } else {
    ...
    /**
     * Stop ASR recording and transcribe. Call from /asrstop or UI.
     */
    fun stopAsrRecordingAndTranscribe() {
        viewModelScope.launch {
            val file = asrRecorder.stopRecording()
            if (file == null) {
                messageManager.addMessage(BitchatMessage(
                    sender = "system",
                    content = "not recording. use /asrstart first.",
                    timestamp = Date(),
                    isRelay = false
                ))
                return@launch
            }
            val transcript = asrEngine.transcribeFile(file)
            messageManager.addMessage(BitchatMessage(
                sender = "you",
                content = transcript,
                timestamp = Date(),
    ...
    fun isAsrRecording(): Boolean = asrRecorder.isRecording()
