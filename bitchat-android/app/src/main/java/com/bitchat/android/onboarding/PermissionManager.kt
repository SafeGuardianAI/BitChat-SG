...
class PermissionManager(private val context: Context) {
     ...
     * Currently includes POST_NOTIFICATIONS on Android 13+ and RECORD_AUDIO for ASR.
     */
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        // Microphone category (optional, for ASR voice input)
        categories.add(
            PermissionCategory(
                type = PermissionType.MICROPHONE,
                description = "Use voice input for speech-to-text (ASR)",
                permissions = listOf(Manifest.permission.RECORD_AUDIO),
                isGranted = isPermissionGranted(Manifest.permission.RECORD_AUDIO),
                systemDescription = "Allow bitchat to record audio for voice commands"
            )
        )
