...
object FunctionRegistry {
    ...
    val functions: List<FunctionDef> = listOf(
        ...
        FunctionDef(
            name = "get_nearest_fm_station",
            description = "Find the nearest emergency FM broadcast station to the user's current location. Use when the user asks what FM frequency to listen to for emergency news, or when there is no internet/mesh connection.",
            params = listOf(
                FunctionParam("latitude", "number", "User's current latitude in decimal degrees"),
                FunctionParam("longitude", "number", "User's current longitude in decimal degrees")
            ),
            requiresConfirmation = false
        )
    )
