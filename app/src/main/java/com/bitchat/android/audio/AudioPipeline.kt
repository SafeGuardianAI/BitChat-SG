...
class AudioPipeline(private val context: Context) {
    ...
    /** Set to true while AsrAudioRecorder holds the mic; pipeline read loops yield. */
    @Volatile
    private var asrHoldsMic = false
    ...
    /**
     * Notify the pipeline that AsrAudioRecorder has taken the microphone.
     * While [active] == true the read loops yield and do not compete for the mic.
     */
        ...
        pipelineJob = scope.launch(Dispatchers.IO) {
            ...
            try {
                while (isActive && isRunning) {
                    if (asrHoldsMic) { delay(50); continue }
