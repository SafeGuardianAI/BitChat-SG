...
object DistributedMemoryProtocol {
    ...
    fun <T> decodePayload(message: ProtocolMessage, clazz: Class<T>): T? {
        return try {
            gson.fromJson(message.payload, clazz)
        } catch (e: Exception) {
            Log.w(TAG, "Failed to decode payload as ${clazz.simpleName}", e)
            null
        }
    }

    /** Reified convenience overload (non-inline to avoid visibility issues). */
    inline fun <reified T> decodePayload(message: ProtocolMessage): T? {
        return decodePayload(message, T::class.java)
    }
