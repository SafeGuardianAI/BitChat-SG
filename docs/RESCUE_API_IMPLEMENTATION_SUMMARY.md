...
📁 Location: `app/src/main/java/com/bitchat/android/ai/RescueAPIService.kt`
...
### 2. **AIService.kt**
📁 Location: `app/src/main/java/com/bitchat/android/ai/AIService.kt`
...
VictimInfo Extraction
    ↓
RescueAPIService
    ├─→ Backend: MongoDB (victim_data)
    ├─→ Backend: Firebase (victim_info)
    └─→ Network Error
        ↓
...
```
/structured [on|off]
├─ OFF: Disable structured output
└─ ON: Enable (uses current mode)

/structured-type [mode]
├─ OFF: No constraints
├─ PROMPT: JSON via system prompt
└─ GRAMMAR: GBNF enforcement (recommended)
```
